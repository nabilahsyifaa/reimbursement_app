...
// Query dasar, ditambahkan filter berdasarkan user login
$sql = "SELECT 
    l.id_log,
    l.id_pengajuan,
    u.nama_lengkap,
...
// Eksekusi query dengan bind user_id dari session
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id); // user_id dari session
$stmt->execute();
...
<body>
  ...
  <small><?= htmlspecialchars($namaPosisi) ?></small>
    ...
    <a href="daftar_pekerjaan_employee.php">Daftar Pekerjaan</a>
  ...
  Hai, selamat datang! Berikut ini merupakan status pengajuan reimbursement yang Anda ajukan.
</div>
