...
// Aksi log
$aksiRes = $conn->query("SELECT id_aksi, nama_aksi FROM aksi WHERE id_role = 4");
while ($row = $aksiRes->fetch_assoc()) {
    $aksiList[] = $row;
}

// Proses form keputusan Finance
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($updateSuccess) {
        ...
        if ($id_aksi == 4) {
            $id_aktifitas_finance = 4;

 $id_finance = $data['id_user']; // user yang mengajukan reimbursement
...
$stmtLog->close();
        ...
        header("Location: daftar_pekerjaan_finance.php");
        exit;
    } else {
...
$stmtLogs = $conn->prepare("SELECT l.*, 
                            ...
                            ORDER BY (l.id_aktifitas = 2) DESC, l.created_at ASC");
